---
id: linear_algebraic_groups.representation_equals_comodule
...
[[node:linear_algebraic_groups.coordinate_hopf_algebra_of_group_scheme|coordinate Hopf algebra]]
\(A = \mathcal{O}(G)\), and \(V\) a \(k\)-vector space. The following
data are naturally equivalent:

1. an [[node:linear_algebraic_groups.comodule_definition|$A$-comodule]]
