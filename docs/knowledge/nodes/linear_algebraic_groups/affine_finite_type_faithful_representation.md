...
1. Apply the
   [[node:affine_group_schemes.hopf_spec_essential_image_affine|Hopf algebra correspondence]]
   to identify \(G\) with \(\operatorname{Spec} A\) for a finitely
   generated commutative Hopf \(k\)-algebra \(A\).
2. Apply
   [[node:affine_group_schemes.finite_dim_subcomodule_lemma|finite generation of comodules]]
